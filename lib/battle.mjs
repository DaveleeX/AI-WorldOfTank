...
export function stats(def,gear={}) {let b=EQUIPMENT.barrel[gear.barrel||0],t=EQUIPMENT.tracks[gear.tracks||0],a=EQUIPMENT.ammo[gear.ammo||0],r=EQUIPMENT.armor[gear.armor||0];return {...def,hp:Math.round(def.hp*r.hp),damage:Math.round(def.damage*b.damage*a.damage),reload:def.reload*b.reload,speed:def.speed*t.speed,maxEnergy:100*t.energy,splash:a.splash||0,offroad:gear.tracks===1};}
export function makeObstacles(map){let a=[];let seed=42;const rnd=()=>{seed=(seed*1664525+1013904223)>>>0;return seed/4294967296};for(let i=0;i<68;i++){let x=(rnd()-.5)*330,z=(rnd()-.5)*290;if(Math.abs(x)<15||Math.abs(z)>110&&Math.abs(x)<65)continue;let building=map==='city'||(map==='mud'&&i%3===0);a.push({x,z,w:building?10+rnd()*10:5+rnd()*10,d:building?10+rnd()*10:5+rnd()*9,h:building?(map==='mud'?4+rnd()*3:8+rnd()*19):3+rnd()*7,type:building?'building':map==='snow'?'ice':'rock',tone:rnd()});}return a;}
...
export class Battle {
 constructor(tank=0,map=0,gear={}){this.map=MAPS[map];this.obstacles=makeObstacles(this.map.id);this.tanks=[];this.shots=[];this.events=[];this.time=0;this.result=null;this.kills=0;this.damage=0;this.rng=735;for(let team=0;team<2;team++)for(let i=0;i<5;i++){let def=TANKS[team===0&&i===0?tank:(i+team*2)%6];let s=stats(def,team===0&&i===0?gear:{});this.tanks.push({id:team*5+i,team,def,s,x:(i-2)*18,z:team?124:-124,yaw:team?Math.PI:0,aim:team?Math.PI:0,hp:s.hp,energy:s.maxEnergy,reload:i*.22,speed:0,alive:true,avoid:0,stuck:0,phase:i*1.2});}this.player=this.tanks[0];}
 ...
 fire(t){if(!t.alive||t.reload>0||this.result)return false;t.reload=t.s.reload*(t.id===0?1:1.65);let dx=Math.sin(t.aim),dz=Math.cos(t.aim);this.shots.push({x:t.x+dx*2,z:t.z+dz*2,dx:dx*150,dz:dz*150,owner:t.id,team:t.team,damage:t.s.damage*(t.id===0?1:.58),splash:t.s.splash,life:2.6});this.events.push({type:'fire',x:t.x+dx*5,z:t.z+dz*5,owner:t.id});return true;}
 hit(v,d,owner){if(!v.alive)return;let damage=Math.min(v.hp,d);v.hp=Math.max(0,v.hp-d);if(owner===0)this.damage+=damage;this.events.push({type:'hit',x:v.x,z:v.z,damage:Math.round(d),target:v.id,owner});if(v.hp<=0){v.alive=false;v.speed=0;if(owner===0)this.kills++;this.events.push({type:'kill',x:v.x,z:v.z,target:v.id,owner});}}
 update(dt,input={}){if(this.result)return;dt=Math.min(dt,.05);this.time+=dt;for(let t of this.tanks){if(!t.alive)continue;t.reload=Math.max(0,t.reload-dt);if(t.id===0){t.aim=input.aim??t.aim;let turn=(input.left?1:0)-(input.right?1:0);t.yaw+=turn*1.55*dt;let throttle=(input.forward?1:0)-(input.back?1:0);let boost=input.boost&&t.energy>0&&throttle>0;t.energy=clamp(t.energy+(boost?-30:18)*dt,0,t.s.maxEnergy);let grip=t.s.offroad?(1+this.map.grip)/2:this.map.grip;let desired=throttle*t.s.speed*grip*(boost?1.65:1)*(throttle<0?.6:1);t.speed+=(desired-t.speed)*Math.min(1,dt*4);this.move(t,Math.sin(t.yaw)*t.speed*dt,Math.cos(t.yaw)*t.speed*dt);if(input.fire)this.fire(t);
 }else{let enemies=this.tanks.filter(v=>v.team!==t.team&&v.alive);enemies.sort((a,b)=>Math.hypot(a.x-t.x,a.z-t.z)-Math.hypot(b.x-t.x,b.z-t.z));let target=enemies.find(v=>this.clearLine(t,v))||enemies[0];if(!target)continue;let dx=target.x-t.x,dz=target.z-t.z,dist=Math.hypot(dx,dz);let visible=this.clearLine(t,target);t.aim=Math.atan2(dx,dz);if(visible&&dist<135&&t.reload<=0){let aim=t.aim;t.aim+=(this.random()-.5)*.06;this.fire(t);t.aim=aim;}
 ...
 for(let s of this.shots){if(s.life<=0)continue;let dx=s.dx*dt,dz=s.dz*dt,min=1,target=null,impact=false;for(let o of this.obstacles){let u=segmentBox(s.x,s.z,dx,dz,o);if(u!==null&&u<min){min=u;impact=true;target=null;}}for(let t of this.tanks){if(!t.alive||t.team===s.team)continue;let u=segmentCircle(s.x,s.z,dx,dz,t.x,t.z,2.5);if(u!==null&&u<min){min=u;target=t;impact=true;}}s.x+=dx*min;s.z+=dz*min;s.life-=dt;if(impact){s.life=0;this.events.push({type:'impact',x:s.x,z:s.z});if(target)this.hit(target,s.damage,s.owner);if(s.splash)for(let t of this.tanks)if(t!==target&&t.alive&&t.team!==s.team&&Math.hypot(t.x-s.x,t.z-s.z)<s.splash&&this.clearLine({x:s.x,z:s.z},t))this.hit(t,s.damage*.45,s.owner);}}

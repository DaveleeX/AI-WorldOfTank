...
export class Battle {
 constructor(tank=0,map=0,gear={}){this.map=MAPS[map];this.obstacles=makeObstacles(this.map.id);this.tanks=[];this.shots=[];this.events=[];this.time=0;this.result=null;this.kills=0;this.damage=0;this.rng=735;for(let team=0;team<2;team++)for(let i=0;i<5;i++){let def=TANKS[team===0&&i===0?tank:(i+team*2)%6];let s=stats(def,team===0&&i===0?gear:{});this.tanks.push({id:team*5+i,team,def,s,x:(i-2)*18,z:team?124:-124,yaw:team?Math.PI:0,aim:team?Math.PI:0,pitch:0,hp:s.hp,energy:s.maxEnergy,reload:i*.22,speed:0,alive:true,avoid:0,stuck:0,phase:i*1.2});}this.player=this.tanks[0];}
 ...
 fire(t){if(!t.alive||t.reload>0||this.result)return false;
 t.reload=t.s.reload*(t.id===0?1:1.65);const p=muzzle(t),c=Math.cos(t.pitch),v=t.s.muzzleSpeed;
 ...
 const delta={x:p.x-base.x,y:p.y-base.y,z:p.z-base.z};let obstruction=1;
 for(const o of this.obstacles){const u=segmentAABB(base,delta,{x:o.x-o.w/2,y:0,z:o.z-o.d/2},{x:o.x+o.w/2,y:o.h??20,z:o.z+o.d/2});if(u!==null)obstruction=Math.min(obstruction,u);}
 if(obstruction<1){this.events.push({type:'impact',x:base.x+delta.x*obstruction,y:base.y+delta.y*obstruction,z:base.z+delta.z*obstruction});return true;}
 this.shots.push({...p,dx:Math.sin(t.aim)*c*v,dy:Math.sin(t.pitch)*v,dz:Math.cos(t.aim)*c*v,owner:t.id,team:t.team,damage:t.s.damage*(t.id===0?1:.58),splash:t.s.splash,life:8,ammo:t.s.ammo});
 this.events.push({type:'fire',...p,owner:t.id});return true;}

 hit(v,d,owner){if(!v.alive)return;let damage=Math.min(v.hp,d);v.hp=Math.max(0,v.hp-d);if(owner===0)this.damage+=damage;this.events.push({type:'hit',x:v.x,z:v.z,damage:Math.round(d),target:v.id,owner});if(v.hp<=0){v.alive=false;v.speed=0;if(owner===0)this.kills++;this.events.push({type:'kill',x:v.x,z:v.z,target:v.id,owner,kills:owner===0?this.kills:0});}}
 ...
 let remaining=dt;while(remaining>1e-8&&s.life>0){const h=Math.min(remaining,1/120);remaining-=h;const next=shellPosition(s,h),d={x:next.x-s.x,y:next.y-s.y,z:next.z-s.z};let min=1,target=null,impact=false;
 for(const o of this.obstacles){const u=segmentAABB(s,d,{x:o.x-o.w/2,y:0,z:o.z-o.d/2},{x:o.x+o.w/2,y:o.h??20,z:o.z+o.d/2});if(u!==null&&u<=min){min=u;impact=true;target=null;}}
 ...
 const u=segmentAABB(local,ld,{x:-2.05,y:.1,z:-2.65},{x:2.05,y:2.85,z:2.65});if(u!==null&&u<min){min=u;target=t;impact=true;}}
 if(next.y<=0&&s.y>=0){const u=s.y/(s.y-next.y);if(u<=min){min=u;target=null;impact=true;}}
 s.x+=d.x*min;s.y+=d.y*min;s.z+=d.z*min;s.dy-=GRAVITY*h;s.life-=h;
 if(impact){s.life=0;this.events.push({type:'impact',x:s.x,y:Math.max(0,s.y),z:s.z});if(target)this.hit(target,s.damage,s.owner);

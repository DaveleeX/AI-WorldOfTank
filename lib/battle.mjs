...
export class Battle {
 ...
 if(obstruction<1){const p={x:base.x+delta.x*obstruction,y:base.y+delta.y*obstruction,z:base.z+delta.z*obstruction};this.events.push({type:'impact',...p});this.damageBuilding(cover,p);return true;}
 ...
 if(impact){s.life=0;this.events.push({type:'impact',x:s.x,y:Math.max(0,s.y),z:s.z});if(target)this.hit(target,s.damage,s.owner);if(cover)this.damageBuilding(cover,s);

...
export class TankRenderer {
 ...
 ...['barrel_1','barrel_2','tracks_1_5','tracks_1_6','tracks_1_7','tracks_2_5','tracks_2_6','tracks_2_7','armor_1','armor_2','ammo_0','ammo_1','ammo_2','aircraft'].map(async key=>{const g=await new GLTFLoader().loadAsync('/equipment/'+key+'.glb');if(!this.active){this.disposeObject(g.scene);return;}this.equipment[key]=g.scene;})
 ...
 if(gear.tracks)tracks.add(this.cloneAsset(this.equipment?.['tracks_'+gear.tracks+'_'+ROAD_WHEELS[def.model]]));
 ...
 root.traverse(o=>{if(o.isMesh){o.layers.enable(1);for(const m of Array.isArray(o.material)?o.material:[o.material])finishVehicleMaterial(m,def.model);}});root.userData={turret,body,cannon,tracks,spec};return root;
 }
 ...
 async buildMap(index,garage=false){
  ...
  this.sun.color.set(d.id==='snow'?0xdcecff:0xffe2c6);this.sun.intensity=d.id==='mud'?2.5:2.9;
 ...
 if(this.focus==='ammo'){pos.set(5.7,2.3,4.4);target.set(3,.9,1.2);fov=36;}else{const v=views[this.focus];pos.fromArray(v[0]).applyAxisAngle(UP,this.preview.rotation.y);target.fromArray(v[1]).applyAxisAngle(UP,this.preview.rotation.y);fov=this.focus==='barrel'?44:41;pos.sub(target).multiplyScalar(this.focus==='barrel'?1.3:1.12).add(target);}}
 if(!this.flyover){if(this.focus)this.camera.setViewOffset(this.host.clientWidth,this.host.clientHeight,this.host.clientWidth*.14,0,this.host.clientWidth,this.host.clientHeight);else this.camera.clearViewOffset();const smooth=1-Math.exp(-dt*3.5);this.camera.position.lerp(pos,smooth);this.lookTarget.lerp(target,smooth);this.camera.fov+=(fov-this.camera.fov)*smooth;this.camera.updateProjectionMatrix();this.camera.lookAt(this.lookTarget);}
 ...
 for(const s of b.shots){const m=this.mesh(new THREE.BoxGeometry(.10,.10,2.2),new THREE.MeshBasicMaterial({color:[0xffe8a8,0x96e9ff,0xff824e][s.ammo||0]}),this.world,s.x,s.y,s.z);m.quaternion.setFromUnitVectors(new THREE.Vector3(0,0,1),new THREE.Vector3(s.dx,s.dy,s.dz).normalize());this.shotMeshes.push(m);}
 }
 if(this.flyover)this.renderFlyover(dt);
 for(const e of this.effects){e.life-=dt;e.mesh.position.addScaledVector(e.v,dt);e.v.y-=9.81*dt;e.mesh.material.opacity=Math.max(0,e.life/e.max);e.mesh.scale.multiplyScalar(1+dt);if(e.life<=0){this.world.remove(e.mesh);e.mesh.geometry.dispose();e.mesh.material.dispose();}}this.effects=this.effects.filter(e=>e.life>0);this.renderer.render(this.scene,this.camera);
 }

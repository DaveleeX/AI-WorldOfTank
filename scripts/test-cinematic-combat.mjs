...
try{
 ...
 for(const k of ['barrel_1','barrel_2','tracks_1','tracks_2','armor_1','armor_2','ammo_0','ammo_1','ammo_2','aircraft'])e.equipment[k]=await load('equipment/'+k+'.glb');
 ...
 e.world=new THREE.Group();e.mode='garage';e.gear={};e.selected=0;e.focus=null;e.setTank(0);e.setFocus('barrel');e.previewEquipment('barrel',1);assert.equal(e.preview.userData.spec.length,4.4);assert.equal(e.gear.barrel,undefined);e.setFocus('ammo');assert(e.ammoDisplay);e.setFocus(null);assert.equal(e.ammoDisplay,null);assert.equal(e.preview.userData.spec.length,3.65);pass('Equipment preview is reversible and never purchases or alters equipped gear');
